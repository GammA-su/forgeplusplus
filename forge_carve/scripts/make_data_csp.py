...
@app.command()
def main(n: int = 50, seed: int = 123, out: str = "out/data/csp.jsonl") -> None:
    configure_logging()
    logger = get_logger(__name__)
    configure_runtime(logger)
    logger.info("data_gen domain=csp n=%d seed=%d out=%s", n, seed, out)
    examples = generate_dataset("csp", n=n, seed=seed)
